...
window.addEventListener("DOMContentLoaded", function () {
  ...
  if (!counters) return; // function 'debounce' is used here for better performance when scroll event fires
  ...
  function textInsideFrontEnd(value, isShowSeparator, separator) {
    return isShowSeparator === "true" ? value.toString().replace(/\B(?=(\d{3})+(?!\d))/g, separator) : value.toString();
  }
  ...
  counters.forEach(function (counter) {
    ...
    function updateCount() {
      x += increaseBy;
      counter.innerText = textInsideFrontEnd(Math.floor(x), isShowSeparator, separator);
      ...
      if (x < target) {
        ...
        counter.innerText = textInsideFrontEnd(target, isShowSeparator, separator);
      }
    }
